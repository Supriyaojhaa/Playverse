...
class Game{
    ...
    init(){//position each block in its proper position
        for(let y=0;y<this.rows;y++){
            for(let x=0;x<this.cols;x++){
                ...
                block.addEventListener('click',(e)=>this.onClickOnBlock(blockIdx)); //Adds a click handler to the block
                this.indexes.push(blockIdx);
            }
        }
        this.indexes.push(this.count-1); //Adds the "empty" block index at the end
        this.randomize(this.difficulty);
    }

    randomize(iterationCount){//move a random block (x iterationCount) Takes how many moves to attempt
        for(let i=0;i<iterationCount;i++){
            let randomBlockIdx=Math.floor(Math.random()*(this.count-1));
            let moved=this.moveBlock(randomBlockIdx);
            if(!moved)i--;  // if move failed, retry
        }
    }
    ...
    moveBlock(blockIdx){//moves a block and return true if the block has moved
        ...
        if(blockCoords!=null){
            ...
            this.indexes[this.emptyBlockCoords[0]+this.emptyBlockCoords[1]*this.cols]=this.indexes[blockCoords[0]+blockCoords[1]*this.cols]; //Updates the indexes[] array to reflect the new position.
    ...
    setDifficulty(difficultyLevel){//set difficulty and reshuffles the puzzle
        this.difficulty=GameDifficulty[difficultyLevel-1];
        this.randomize(this.difficulty);
    }
